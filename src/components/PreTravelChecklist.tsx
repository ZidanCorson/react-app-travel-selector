  ...
  return (
    <div className="card shadow-sm h-100">
      <div className="card-header bg-primary text-white">
        <h5 className="mb-0"><i className="bi bi-clipboard-check me-2"></i>Pre-Travel Requirements for {city}</h5>
      </div>
      <div className="card-body">
        ...
        <div className="alert alert-info border-0 bg-light-subtle mb-4">
          <div className="mb-2">
            <strong><i className="bi bi-passport me-2"></i>Visa:</strong> {info.visa}
          </div>
          <div className="mb-2">
            <strong><i className="bi bi-activity me-2"></i>Health:</strong> {info.health}
          </div>
          <div>
            <strong><i className="bi bi-plug me-2"></i>Power:</strong> {info.electrical}
          </div>
        </div>

        {/* Checklist Section */}
        <h6 className="border-bottom pb-2 mb-3">Essential Tasks</h6>
        <div className="list-group list-group-flush">
          {defaultTasks.map((task, index) => (
            <label key={index} className="list-group-item d-flex gap-3 align-items-center cursor-pointer border-0 px-0 py-2">
              <input 
              ...
              />
              <span className={checkedItems.includes(task) ? "text-decoration-line-through text-muted" : ""}>
                {task}
              </span>
            </label>
          ))}
        ...
        {checkedItems.length === defaultTasks.length && (
          <div className="mt-3 text-center text-success animate__animated animate__fadeIn">
             <i className="bi bi-check-circle-fill me-1"></i> All set! You are ready to go.
          </div>
        )}
      </div>
    </div>
